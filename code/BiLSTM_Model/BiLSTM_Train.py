...
def train_bilstm(
    ...
    for epoch in range(start_epoch, start_epoch + num_epochs):
        ...
        if should_validate:
            val_loss, val_token_acc, val_board_acc = evaluate_bilstm(
                model,
                val_loader,
                device,
            )
        ...
        save_checkpoint(
            ...
            scheduler,
            epoch,
            f"{checkpoint_dir}/bilstm_last.pt",
        ...
        if metric_for_best > best_metric:
            ...
            save_checkpoint(
                ...
                scheduler,
                epoch,
                f"{checkpoint_dir}/bilstm_best.pt",
                best_board_acc=best_metric,
            )
        ...
        if checkpoint_every is not None and (epoch + 1) % checkpoint_every == 0:
            save_checkpoint(
                ...
                scheduler,
                epoch,
                f"{checkpoint_dir}/bilstm_epoch_{epoch + 1}.pt",
                best_board_acc=best_metric,
            )
